...
export class Player {
    constructor(scene, camera, world, initialPosition, particleSystem, soundManager, networkManager, characterClass = 'witch') {
        ...
        if (this.characterClass !== 'warlock') {
    ...
    update(delta, remotePlayers) {
        ...
        // Desired Offset from neck to camera (Right 1.5, Up 1.0, Back 5)
        const desiredOffset = new THREE.Vector3(1.5, 1.0, 5);
        desiredOffset.applyEuler(this.cameraRotation);
        
        const desiredCameraPos = pivot.clone().add(desiredOffset);

        // Camera Collision Check
        // Raycast from pivot to desiredCameraPos
        const camDirection = desiredCameraPos.clone().sub(pivot).normalize();
        const distance = pivot.distanceTo(desiredCameraPos);
        
        // Use world raycast to find wall
        const hit = this.world.raycast(pivot, camDirection, distance);
        
        if (hit) {
            // If hit, place camera slightly in front of the wall
            // Move 0.2 units closer to player from hit point to avoid clipping into wall
            const hitDist = pivot.distanceTo(hit.point);
            const safeDist = Math.max(0.5, hitDist - 0.2); // Keep at least 0.5 away from head
            
            const safePos = pivot.clone().add(camDirection.multiplyScalar(safeDist));
        ...
        const lookTarget = this.camera.position.clone().add(viewDirection.multiplyScalar(20));
    ...
    createWarlockMesh() {
        ...
        const skinColor = 0xffccaa;
        const robeColor = 0x4b0082; // Indigo/Purple
        const hatColor = 0x4b0082; // Matching Purple
        ...
        const legMat = new THREE.MeshStandardMaterial({ color: robeColor }); // Pants match robe or black? Image shows purple robe covering mostly.
        const bootMat = new THREE.MeshStandardMaterial({ color: bootColor });
